...
exports.scoreCarrierForShipment = scoreCarrierForShipment;
...
exports.calculateAverageShipmentDistance = calculateAverageShipmentDistance;
exports.groupShipmentsByStatus = groupShipmentsByStatus;
exports.findTopCarriers = findTopCarriers;
...
function scoreCarrierForShipment(carrier, shipment, product) {
    ...
    if (carrier.maxWeightKg >= product.weightKg * shipment.quantity)
        score += 20;
    // Supports priority
    ...
    if (!product.isFragile)
        score += 15;
    else if (product.isFragile && carrier.handlesFragile)
        score += 15;
    // Reliability
...
function calculateTotalInventoryValue(products) {
    const total = products.reduce((sum, p) => sum + p.stockQuantity * p.unitCostUSD, 0);
    return Math.round(total * 100) / 100;
}
function calculateAverageShipmentDistance(shipments) {
    if (shipments.length === 0)
        return 0;
    const total = shipments.reduce((sum, s) => sum + s.destination.distanceKm, 0);
