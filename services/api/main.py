...
def health_check():
    return {
        "status": "ok",
        "service": "TrackFlow Incident Analyzer API",
    }
...
async def analyze_incidents(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                "The CSV file could not be analyzed. "
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "We could not analyze the file right now. "
...
def export_results():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "We could not export the analysis results "
